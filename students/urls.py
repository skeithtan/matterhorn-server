...
student_urls = [
    ...
    url(r'(?P<student_id>(\d+))/programs/$', StudentStudyFieldListCreateView.as_view()),
    url(r'(?P<student_id>(\d+))/programs/(?P<study_field_id>(\d+))/$',
        StudentStudyFieldRetrieveUpdateDestroyView.as_view()),
...
student_programs_archived_urls = [
    url(r'(?P<pk>(\d+))/restore/$', StudentStudyFieldRestoreView.as_view()),
    url(r'(?P<pk>(\d+))/$', ArchivedStudentStudyFieldUpdateView.as_view()),
